  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .ws-toast {
      ...
      background: #16a34a; color: #fff; border-radius: 18px;
      padding: 14px 24px; font-size: .95rem; font-weight: 700;
      box-shadow: 0 20px 50px rgba(0,0,0,.4); z-index: 500;
      display: flex; align-items: center; gap: 10px;
      animation: toastIn .4s cubic-bezier(.34,1.56,.64,1);
      white-space: nowrap;
    }
    @keyframes toastIn { from{opacity:0;transform:translateX(-50%) translateY(20px)} to{opacity:1;transform:translateX(-50%) translateY(0)} }
    .ws-toast.out { animation: toastOut .3s ease forwards; }
    ...
    .step-dot.active { background: var(--primary); border-color: var(--primary); box-shadow: 0 0 8px rgba(249,115,22,.6); }
    .step-dot.done   { background: #22c55e; border-color: #22c55e; }
    .step-line { flex: 1; height: 2px; background: rgba(255,255,255,.15); border-radius: 1px; transition: background .3s; }
...
<body>
  ...
  <div class="status-bar" id="statusBar">
    <div class="status-bar-title">
      🍽️ Seu Pedido
      <span id="sbPedidoId" style="font-weight:700;color:var(--primary);margin-left:6px">—</span>
    </div>
    <div class="status-bar-row" style="margin-top:4px">
      <span id="sbBadge" class="status-badge badge-Pendente">Pendente</span>
      <span id="sbTempo" class="status-tempo"></span>
    </div>
    <div id="sbProgresso" style="display:flex;gap:4px;margin-top:10px;align-items:center">
      <div class="step-dot active" data-step="Pendente" title="Recebido"></div>
      <div class="step-line"></div>
      <div class="step-dot" data-step="Preparando" title="Preparando"></div>
      <div class="step-line"></div>
      <div class="step-dot" data-step="Saiu" title="Saiu para entrega"></div>
      <div class="step-line"></div>
      <div class="step-dot" data-step="Entregue" title="Entregue"></div>
    </div>
    <div id="sbStepLabel" style="font-size:.72rem;color:var(--text-soft);margin-top:6px">Pedido recebido, aguardando preparo...</div>
    <div class="status-bar-row" style="margin-top:6px">
      <span id="sbMesa" class="status-tempo" style="font-size:.78rem"></span>
    </div>
  </div>
  ...
  <script>
    ...
    async function confirmarPedido() {
      ...
      try {
        ...
        if (data.sucesso) {
          ...
          document.getElementById('heroItensText').textContent = `Pedido #${data.pedido_id} enviado`;

          // ✅ TOAST de confirmação
          mostrarToast(`✅ Pedido #${String(data.pedido_id).padStart(3,'0')} confirmado! Acompanhe o andamento abaixo.`);
        } else {
    ...
    async function atualizarStatus() {
      ...
      try {
        ...
        sbBadge.className      = `status-badge badge-${data.status}`;
        sbBadge.textContent    = data.status;
        sbMesa.textContent     = data.mesa ? `Mesa ${data.mesa}` : '';
        sbTempo.textContent    = data.tempo_estimado ? `⏱ ~${data.tempo_estimado} min` : '';

        // Atualiza progresso
        atualizarProgresso(data.status);

        // Atualiza hero floating card
        document.getElementById('heroStatusText').textContent = data.status;

        // Para polling se finalizado
        if (data.status === 'Entregue' || data.status === 'Cancelado') {
          clearInterval(pollingTimer);
          mostrarToast(data.status === 'Entregue' ? '✅ Pedido entregue! Bom apetite! 😊' : '❌ Pedido cancelado.');
        }
      } catch(e) {
    ...
    /* ──────────────────────────────────── PROGRESSO STEPS ── */
    const stepOrder = ['Pendente', 'Preparando', 'Saiu', 'Entregue'];
    const stepLabels = {
      'Pendente':   '⏳ Pedido recebido, aguardando o preparo...',
      'Preparando': '👨‍🍳 Seu pedido está sendo preparado na cozinha!',
      'Saiu':       '🛵 Pedido saiu para entrega / pronto para retirada!',
    ...
    function atualizarProgresso(status) {
      ...
      const labelEl = document.getElementById('sbStepLabel');
      if (labelEl) labelEl.textContent = stepLabels[status] || status;
    }
    ...
    async function verificarLogin() {
      try {
        ...
        if (dadosLogin.logado) {
          area.innerHTML = `
            <span class="usuario-logado">Olá, ${dadosLogin.nome}</span>
